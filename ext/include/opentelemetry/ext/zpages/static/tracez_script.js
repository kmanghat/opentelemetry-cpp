...
const getLatencyCell = (span, i, h) =>
  `<td${
    span[h][i] === 0
      ? ""
      : ` class='click'
          onclick="overwriteDetailedTable(${i}, '${span["name"]}')"`
  }>${span[h][i]}</td>`;
...
const detailCols = [
  "attributes",
]; /* Columns error, running, and latency spans all share */
...
const getCellHeadings = (group) =>
  "cell_headings" in getFormat(group)
    ? getFormat(group)["cell_headings"]
    : getHeadings(group);
const getSizing = (group) => getFormat(group)["sizing"];
const getStatus = (group) =>
  isLatency(group) ? "ok" : getFormat(group)["status"];
const getHTML = (group) => getFormat(group)["html_id"];

const isDate = (col) => new Set(dateCols).has(col);
const isLatency = (group) =>
  !new Set(clickCols).has(
    group
  ); /* non latency clickable cols, 
    change to include latency? */
const isArrayCol = (group) => new Set(Object.keys(arrayCols)).has(group);
const hasCallback = (col) =>
  new Set(clickCols).has(col); /* Non-latency cb columns */
const hideHeader = (h) =>
  new Set([...clickCols, "name"]).has(
    h
  ); /* Headers to not show render 
    twice */
const hasSubheading = (group) =>
  isLatency(group) || "has_subheading" in getFormat(group);
...
const updateLastRefreshStr = () =>
  (document.getElementById(
    "lastUpdateTime"
  ).innerHTML = new Date().toLocaleString());

const getStatusHTML = (group) =>
  !hasStatus(group)
    ? ""
...
/* Returns an HTML string that handlles width formatting
     for a table group */
const tableSizing = (group) =>
  "<colgroup>" +
  getSizing(group)
    .map((sz) => `<col class='${sz["sz"]}'></col>`.repeat(sz["repeats"]))
    .join("") +
  "</colgroup>";

/* Returns an HTML string for a table group's headings,
     hiding headings where needed */
const tableHeadings = (group) =>
  "<tr>" +
  getCellHeadings(group)
    .map((h) => `<th>${hideHeader(h) ? "" : h}</th>`)
    .join("") +
  "</tr>";
...
const getArrayCells = (h, span) =>
  span[h].length
    ? span[h].map((_, i) => arrayCols[h](span, i, h)).join("")
    : Object.keys(span[h]).length
    ? arrayCols[h](span, h)
    : `<td>${emptyContent()}</td>`;
...
const dateStr = (nanosec) => {
  ...
  return `${mainDate.substr(
    0,
    endingLocation
  )}:${lostPrecision}${mainDate.substr(endingLocation)}`;
};
...
const getCell = (h, span) =>
  isArrayCol(h)
    ? getArrayCells(h, span)
    : `<td ${
        hasCallback(h) && span[h] !== 0
          ? `class='click'
...
const tableRow = (group, span) =>
  "<tr>" +
  getHeadings(group)
    .map((h) => getCell(h, span))
    .join("") +
  "</tr>";
...
const tableRows = (group, data) =>
  data.map((span) => tableRow(group, span)).join("");
...
function overwriteTable(group, url_end = "") {
  fetch(getURL(group) + url_end)
    .then((res) => res.json())
    .then((data) => {
      document.getElementById(getHTML(group)).innerHTML =
        tableHeader(group) + tableRows(group, data);
      document.getElementById(getHTML(group) + "_count").innerHTML =
        data.length;
    })
...
function updateSubheading(group, name) {
  if (hasSubheading(group)) {
    document.getElementById(
      getHTML(isLatency(group) ? "latency" : group) + "_header"
    ).innerHTML = `<h2><span class='subhead-name'>${name}</span>
                ${
                  isLatency(group)
                    ? `${latencies[group]} Bucket`
                    : toTitlecase(group)
                }
...
function addToTable(group, url_end = "") {
  fetch(getURL(group) + url_end)
    ...
    .then((data) => {
      ...
      document
        .getElementById(getHTML(group))
        .getElementsByTagName("tbody")[0].innerHTML += rowsStr;
    })
    .catch((err) => console.log(err));
}
